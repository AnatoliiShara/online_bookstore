...
from bookstore.errors import BookNotFoundError, DuplicateISBNError
...
def test_add_get_search_and_duplicate(tmp_path):
    ...
    b = Book(
        title="Clean Code", author="Uncle Bob", isbn="9780132350884", price="10.00", quantity=2
    )
    repo.add(b)
    ...
    with pytest.raises(DuplicateISBNError):
        repo.add(Book(title="Another", author="Bob", isbn="9780132350884", price="9.99"))
...
def test_archive_and_remove(tmp_path):
    ...
    """
    repo = make_repo(tmp_path)
    b = Book(title="DDD", author="Evans", isbn="9780321125217", price="15.00", quantity=0)
    repo.add(b)
    ...
    b2 = Book(title="Refactoring", author="Fowler", isbn="9780201485677", price="11.00")
    repo.add(b2)
    repo.remove(b2.id)
